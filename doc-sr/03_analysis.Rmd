...
The catchability parameters appear to show more correlation with each other in this update model than they were in the `r ca`. A comparison of the pairs plots (Figure \@ref(fig:fig-base-pairs)) and Figure 41 from the `r ca` shows that there is a higher correlation between the $q_1$ and $q_3$ parameters in particular. These are the catchability parameters for the `r qcsss` and the `r hsss`.
...
Figures \@ref(fig:fig-catch-streams-nextyr-proj)--\@ref(fig:fig-catch-streams-nextnextnextyr-proj) show the uncertainty in the relative spawning biomass estimates for projected years given the catch values provided in the harvest decision tables. They are essentially a visual depiction of the decision tables. The horizontal lines in the figures are the 95% confidence intervals of the realitive biomass. The probabilities given in the harvest decision tables are close to being the proportion of those lines above each reference point shown as vertical lines in the plots.
