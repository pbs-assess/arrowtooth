...
nms <- names(table_dec)
i_inds <- grep(".*\\{20[0-9]+\\} < B", nms)
i_nms <- nms[i_inds]
i_yrs <- gsub("^.*\\{(20[0-9]+)\\} < B.*$", "\\1", i_nms)

catch_probs_nextyr_curryr <- map(c(0, 5, 10, 15), ~{
  table_dec |> 
    filter(!!catch_col == .x) |>
    pull(i_nms[1])
})
names(catch_probs_nextyr_curryr) <- c(0, 5, 10, 15)

catch_probs_nextnextyr_nextyr <- map(c(0, 5, 10, 15), ~{
  table_dec |> 
    filter(!!catch_col == .x) |>
    pull(i_nms[2])
})
names(catch_probs_nextnextyr_nextyr) <- c(0, 5, 10, 15)

catch_probs_nextnextnextyr_nextnextyr <- map(c(0, 5, 10, 15), ~{
  table_dec |> 
    filter(!!catch_col == .x) |>
    pull(i_nms[3])
})
names(catch_probs_nextnextnextyr_nextnextyr) <- c(0, 5, 10, 15)

probs_proj_less_assess <- table_dec |>
  select(i_nms[1]) |>
  pull()
...
val_less_50_50 <- prob_less_50_50 |> pull(i_nms[1])

# Decision table column indices for P(B_2023<0.4B0) and P(B_2023<0.4B0)
i_nextnextyr_4bo <- grep(paste0("\\{", i_yrs[1], "\\} < 0.4B"), names(table_dec))
i_nextnextyr_2bo <- grep(paste0("\\{", i_yrs[1], "\\} < 0.2B"), names(table_dec))

below_04bo <- table_dec |> 
  pull(i_nextnextyr_4bo)
range_below_04bo <- range(below_04bo)

below_02bo <- table_dec |> 
  pull(i_nextnextyr_2bo)
range_below_02bo <- range(below_02bo)

# For decision table caption text
prob_10t_below_2bo <- table_dec |> 
    filter(!!catch_col == 10) |> pull(i_nextnextyr_2bo)
prob_10t_below_4bo <- table_dec |> 
    filter(!!catch_col == 10) |> pull(i_nextnextyr_4bo)

# For the Projections and Decision table section
uncert_nextnextyr_catch_10 <- base_model$mcmc$proj |>
  filter(TAC == 10) |>
  pull(paste0("B", i_yrs[3])) 
uncert_nextnextyr_catch_10 <- quantile(uncert_nextnextyr_catch_10 /
                                         base_model$mcmc$params$sbo,
                                 probs = c(0.025, 0.5, 0.975))

lo_uncert_nextnextyr_catch_10 <- uncert_nextnextyr_catch_10[1]
hi_uncert_nextnextyr_catch_10 <- uncert_nextnextyr_catch_10[3]
diff_uncert_nextnextyr_catch_10 <- hi_uncert_nextnextyr_catch_10 - lo_uncert_nextnextyr_catch_10
names(diff_uncert_nextnextyr_catch_10) <- "difference"
