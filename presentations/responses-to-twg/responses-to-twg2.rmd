...
points(mesh$loc_xy, pch = 21, col = "#FF000030", cex = 0.2)
mesh$mesh$n
...
m_rw_depth_iso <- sdmTMB(density_kgp100m2 ~ s(log(depth), k = 3L),
  family = tweedie(), 
  data = dat2,
  time = "year", 
  silent = FALSE, 
...
m_rw_aniso_quad <- sdmTMB(
  formula = density_kgp100m2 ~ 1 + depth_scaled + depth_scaled2,
...
saveRDS(m_rw_aniso_quad, file = "~/Downloads/")
saveRDS(m_rw_aniso_quad, here::here("presentations/responses-to-twg/geostat-cached-data/m_rw_aniso_quad.rds"))

m_rw_depth_iso_prior <- sdmTMB(density_kgp100m2 ~ s(log(depth), k = 3L),
  family = tweedie(), 
  data = dat2,
  time = "year", 
  silent = FALSE, 
...
m_rw_quad <- sdmTMB(density_kgp100m2 ~ depth_scaled + depth_scaled2,
  family = tweedie(), 
  data = dat2,
  time = "year", 
  silent = FALSE, 
...
m_rw_yr <- sdmTMB(density_kgp100m2 ~ 0 + as.factor(year) + as.factor(survey_abbrev),
  family = tweedie(), 
  data = dat2,
  time = "year", 
  silent = FALSE, 
...
m_ar1_depth
m_rw_depth
m_rw_depth_iso_prior
m_rw_depth_iso
m_rw_quad

nd2 <- data.frame(depth_scaled =
  seq(min(dat2$depth_scaled), max(dat2$depth_scaled), length.out = 100))
nd2$depth_scaled2 <- nd2$depth_scaled^2
nd2$year <- 2019L
p <- predict(m_rw_aniso_quad, newdata = nd2, se_fit = TRUE, re_form = NA)
ggplot(p, aes(depth_scaled, exp(est), ymin = exp(est - 1 * est_se), ymax = exp(est + 1 * est_se))) +
  geom_line() + 
  geom_ribbon(alpha = 0.4) +
  coord_cartesian(ylim = c(0, max(exp(p$est)) * 2.5), expand = FALSE)

# Visualizing marginal effect 

nd1 <- data.frame(depth =
  seq(min(dat2$depth, na.rm = TRUE), max(dat2$depth, na.rm = TRUE), length.out = 200))
# You'll need at least one time element. If time isn't also a fixed effect
# then it doesn't matter what you pick:
nd1$year <- 2019L
p <- predict(m_rw_depth, newdata = nd1, se_fit = TRUE, re_form = NA)
ggplot(p, aes(depth, exp(est), ymin = exp(est - 1 * est_se), ymax = exp(est + 1 * est_se))) +
  geom_line() + 
  geom_ribbon(alpha = 0.4) +
  coord_cartesian(ylim = c(0, max(p$est) * 2.5), expand = FALSE)
...
nd$depth_scaled <- (log(nd$depth) - dat2$depth_mean[1]) / dat2$depth_sd[1]
...
p_rw_depth_iso <- predict(m_rw_depth_iso, newdata = nd, sims = 400L)
p_rw_quad <- predict(m_rw_quad, newdata = nd, sims = 400L)
p_rw_yr <- predict(m_rw_yr, newdata = nd, sims = 400L)
p_rw_aniso_quad <- predict(m_rw_aniso_quad, newdata = nd, sims = 1000L)

nd$median <- apply(p_rw_aniso_quad, 1, function(x) median(exp(x)))
nd$cv <- apply(p_rw_aniso_quad, 1, function(x) sd(exp(x)) / mean(exp(x)))

filter(nd, year %in% 2000:2019) %>%
  ggplot(aes(X, Y, fill = median, colour = median)) + geom_tile(width = 2.05, height = 2.05) +
  coord_equal() +
  scale_fill_viridis_c(trans = "sqrt") +
  scale_colour_viridis_c(trans = "sqrt") +
  facet_wrap(~year) +
  geom_point(aes(size = density_kgp100m2), 
    data = filter(dat2, year %in% 2000:2019), colour = "white", fill = NA, pch = 21, alpha = 0.2) +
  scale_size_area(max_size = 4)

filter(nd, year %in% 2000:2019) %>%
  ggplot(aes(X, Y, fill = cv, colour = cv)) + geom_tile(width = 2.05, height = 2.05) +
  coord_equal() +
  scale_fill_viridis_c(trans = "log10", option = "C") +
  scale_colour_viridis_c(trans = "log10", option = "C") +
  facet_wrap(~year)  +
  geom_point(aes(size = density_kgp100m2), 
    data = filter(dat2, year %in% 2000:2019), colour = "white", fill = NA, pch = 21, alpha = 0.15) +
  scale_size_area(max_size = 4)
...
filter(nd, year %in% 2000:2019) %>%
  ...
  facet_wrap(~year) 
  # geom_point(aes(size = density_kgp100m2), 
  #   data = filter(dat2, year %in% 2000:2019), colour = "white", fill = NA, pch = 21, alpha = 0.1) +
  # scale_size_area(max_size = 4)


index_ar1 <- get_index_sims(p_ar1)
index_rw_depth <- get_index_sims(p_rw_depth)
index_rw_depth_iso <- get_index_sims(p_rw_depth_iso)
index_rw_quad <- get_index_sims(p_rw_quad)
index_rw_yr <- get_index_sims(p_rw_yr)
index_rw_aniso_quad <- get_index_sims(p_rw_aniso_quad)
...
saveRDS(index_rw_aniso_quad, here::here("presentations/responses-to-twg/geostat-cached-data/index_rw_aniso_quad.rds"))

# scaling <- 1 / (100 * 100) # 100m^2 back to km^2
# scaling <- scaling * 2 * 2 # 2 x 2 km cells
# scaling

head(nd)
pqcs <- p_rw_aniso_quad[which(nd$survey_abbrev == "SYN QCS"), ]
attr(pqcs, "time") <- "year"

scaling <- 10 * 10 * 2 * 2
# index_qcs <- get_index_sims(pqcs)
# index_qcs %>% mutate(biomass_scaled = )

index_rw_aniso_quad %>%
  mutate(cv = sqrt(exp(se^2) - 1)) %>%
  mutate(est = est * scaling, lwr = lwr * scaling, upr = upr * scaling) %>% 
  saveRDS(file = here::here("presentations/responses-to-twg/geostat-cached-data/stitched-syn-index.rds"))

# bind_rows(index_rw_aniso_quad %>% mutate(type = "RW quad aniso")) %>%
index_rw_aniso_quad %>%
  mutate(cv = sqrt(exp(se^2) - 1)) %>%
  mutate(est = est * scaling, lwr = lwr * scaling, upr = upr * scaling) %>% 
...
# index_ar1 %>% mutate(type = "AR1") %>%
  ...
  bind_rows(index_rw_depth_iso %>% mutate(type = "RW depth iso")) %>%
  # bind_rows(index_rw_yr %>% mutate(type = "IID years")) %>%
  bind_rows(index_rw_quad %>% mutate(type = "RW quad iso")) %>% 
  bind_rows(index_rw_aniso_quad %>% mutate(type = "RW quad aniso")) %>% 
