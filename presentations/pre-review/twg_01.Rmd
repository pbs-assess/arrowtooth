...
if(!exists("dat")){
  dat <- readRDS(file.path(nongit_dir, "data", "arrowtooth-flounder-may14-2021.rds"))
}
...
```{r length-freq}
k <- purrr::map2(list(cs_3cd, cs_5ab, cs_5cd, cs_5e),
          c("3CD", "5AB", "5CD", "5E"), ~{
  .x %>%
...
cowplot::plot_grid(gfplot::plot_length_weight(lwf_3cd, lwm_3cd, col = fm_color) +
                     theme(legend.position = "none") +
                     ggtitle(paste0("3CD (", num_f_3cd, " F, ", num_m_3cd, " M)")),
                   gfplot::plot_length_weight(lwf_5ab, lwm_5ab, col = fm_color) +
                     theme(legend.position = "none") +
                     ggtitle(paste0("5AB (", num_f_5ab, " F, ", num_m_5ab, " M)")),
                   gfplot::plot_length_weight(lwf_5cd, lwm_5cd, col = fm_color) +
                     theme(legend.position = "none") +
                     ggtitle(paste0("5CD (", num_f_5cd, " F, ", num_m_5cd, " M)")),
                   gfplot::plot_length_weight(lwf_5e, lwm_5e, col = fm_color) +
                     theme(legend.position = "none") +
                     ggtitle(paste0("5E (", num_f_5e, " F, ", num_m_5e, " M)")),
                   gfplot::plot_length_weight(lwf_coast, lwm_coast, col = fm_color) +
                     theme(legend.position = "none") +
...
```{r survey-age-freq}
j <- purrr::map2(list(survey_samples, ss_3cd, ss_5ab, ss_5cd),
...
- Ages 1- 21+
- Selectivity is estimated up to age 19
- Age at 50% maturity is 7
- Recruitment is estimated for each year, except the last which is the mean
- Age-varying M
- Observation errors for age and length compositions were assumed to be multinomial distributed
- Recruitment deviations and catch and biomass observation errors were assumed to be lognormally distributed
]
