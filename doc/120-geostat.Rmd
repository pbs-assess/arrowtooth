...
Pour les indices des relevés individuels, nous avons utilisé des modèles delta/hurdle (appelés ici modèle $\Delta$-Gamma [@aitchison1955]). Dans ce modèle, la capture de l'étude synoptique (Figures \@ref(fig:geo-bubble1), \@ref(fig:geo-bubble2)) est définie sur la base d'un modèle de probabilité de rencontre et d'un modèle de capture positive.
\begin{equation}
  \mathrm{Pr} [C > 0] = p,
\end{equation}
where $C$ is the observed catch $p$ is the probability of encounter. The positive component given encounter is defined as
...
## INDICES MODÉLISÉS
Les indices géostatistiques pour les relevés individuels avaient des CV plus faibles, en moyenne, que les indices basés sur la conception - en particulier dans le détroit de la Reine-Charlotte (Fig. \@ref(fig:geo-individual)). Les indices cousus $\Delta$-Gamma et Tweedie étaient similaires les uns aux autres. La différence la plus notable est que l'inclusion d'un lisseur pour la profondeur a légèrement réduit l'estimation de la biomasse en 2003-2004 et a rétréci les intervalles de confiance pour ces années (Fig. \@ref(fig:geo-indexes)).

Les indices géostatistiques des relevés couplés à l'échelle de la côte ont tous montré une forte ressemblance avec l'indice de CPUE des rejets commerciaux (Fig. \@ref(fig:geo-indexes-cpue)) avec des intervalles de confiance qui se chevauchent pour la plupart, des déclins marqués de 2010 à 2021, et un creux au milieu des années 2000. La première année de l'étude (2003) a été marquée par une certaine divergence, la CPUE des rejets étant légèrement plus élevée, bien que la majorité des intervalles de confiance se chevauchent toujours.
")
```
...
(ref:fig-geo-bubble1-en) Survey data bubble plot for 2003 to 2012. The area and colour of circles corresponds to set density. Sets with zero `r sp` catch are indicated with a grey cross. 

(ref:fig-geo-bubble1-fr) Graphique à bulles des données d'enquête de 2003 à 2012. La surface et la couleur des cercles correspondent à la densité des calées. Les ensembles dont la prise `r sp` est nulle sont indiqués par une croix grise. 
...
(ref:fig-geo-predmap1-en) Predicted `r sp` biomass density for 2003 to 2012 from the coastwide $\Delta$-Gamma model without depth.

(ref:fig-geo-predmap1-fr) Densité de biomasse `r sp` prédite pour 2003 à 2012 à partir du modèle $\Delta$-Gamma pour l'ensemble de la côte sans profondeur.
...
```{r geo-indexes-cpue, fig.cap = ifelse(fr(), "(ref:fig-geo-indexes-cpue-fr)", "(ref:fig-geo-indexes-cpue-en)"), out.width = "5.5in", results = 'asis'}
