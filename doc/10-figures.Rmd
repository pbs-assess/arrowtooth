...
```{r fig-catch-spatial, fig.cap = ifelse(fr(), "(ref:fig-catch-spatial-fr)", "(ref:fig-catch-spatial-en)")}
plot_catch_spatial(dat$catch_spatial, start_year = base_model$dat$start.yr)
```
...
plot_age_resids_mcmc(base_model, gear = 1)
```
...
plot_age_resids_mcmc(base_model, gear = 2)
```
...
plot_age_fits_mcmc(base_model, gear = 3)
```
...
plot_age_resids_mcmc(base_model, gear = 3)
```
...
plot_age_fits_mcmc(base_model, gear = 4)
```
...
plot_age_resids_mcmc(base_model, gear = 4)
```
...
plot_age_fits_mcmc(base_model, gear = 5)
```
...
plot_age_resids_mcmc(base_model, gear = 5)
```
...
plot_age_fits_mcmc(base_model, gear = 6)
```
...
plot_age_resids_mcmc(base_model, gear = 6)
```
