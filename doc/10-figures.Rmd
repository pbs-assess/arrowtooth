...
```{r fig-ages, fig.cap = ifelse(fr(), "(ref:fig-ages-fr)", "(ref:fig-ages-en)"), fig.asp = 1}
...
sc <- tidy_ages_raw(comm_ss,
  sample_type = "commercial"
) %>% mutate(
  survey_abbrev = "Shoreside"
)

sf <- tidy_ages_raw(comm_ft,
  sample_type = "commercial"
) %>% mutate(
...
```{r fig-mat, fig.cap = ifelse(fr(), "(ref:fig-mat-fr)", "(ref:fig-mat-en)"), fig.asp = 1}
plot_mat_panels <- function(data, type = "age"){
.m <- fit_mat_ogive(data, type = type, sample_id_re = FALSE)
...
```{r fig-lengths, fig.cap = ifelse(fr(), "(ref:fig-lengths-fr)", "(ref:fig-lengths-en)"), fig.asp = 1}
