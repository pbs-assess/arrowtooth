...
Posterior estimates of fishery reference points from the base model are provided in Table \@ref(tab:reference-points-table) and Figure \@ref(fig:fig-base-ref-points). The posterior unfished spawning biomass ($SB_0$) had a median `r f(ref_pts$sbo[2] * 1000)` t and credibility interval ranging from `r f(ref_pts$sbo[1] * 1000)` t to `r f(ref_pts$sbo[3] * 1000)` t (Table \@ref(tab:reference-points-table)). Posterior credibility intervals for the candidate LRP $0.2B_0$ and USR $0.4B_0$ are also provided in Table \@ref(tab:reference-points-table).

Reference points based on maximum sustainable yield ($MSY$) were strongly impacted by estimates of selectivity in the the trawl fisheries described in the previous section. Because the selectivity ogives (Figure \@ref(fig:fig-base-mcmc-selex)) were estimated to be to the right of the maturity ogive (Figure XX), the median estimates of $F_{MSY}$ were `r f(ref_pts$fmsy_fleet1[2], 2)` for the Freezer trawler fleet and `r f(ref_pts$fmsy_fleet2[2], 2)` for the Shoreside fleet (Table \@ref(tab:reference-points-table)). These instantaneous fishing mortalities convert to an annual harvest rate approaching 1 for both fleets, through the equation $U_{MSY} = 1-e^{F_{MSY}}$, implying that all of the vulnerable biomass (i.e., the biomass thatis selected by the fishing gear) could be harvested because the population can be sustained by the spawning biomass that is invulnerable to the fishery (i.e., fish that are between 6 and 9 years old). The relationship between age at maturity and age at first harvest and its effect on fishery reference points was discussed by @myers1998, who described a fishing strategy where overfishing could be avoided by allowing all fish to spawn before they were available to be caught. @froese2004 also discusses reduction in risks of overfishing by allowing fish to spawn before they are caught.

It is important to understand the distinction between vulnerable biomass and spawning biomass. The fishery reference points $F_{MSY}$ and $U_{MSY}$ refer to catch of the vulnerable biomass $VB_t$, which is determined by the selectivity function:
...
The median posterior estimate of $B_{MSY}$ (and 95% credibility interval), conditional on estimated trawl selectivities and resulting $F_{MSY}$ values, was `r f(ref_pts$bmsy[2] * 1000)` t (`r f(ref_pts$bmsy[1] * 1000)`-`r f(ref_pts$bmsy[3] * 1000)`) (Table \@ref(tab:reference-points-table)). Posterior credibility intervals for the default LRP $0.4B_{MSY}$ and USR $0.8B_{MSY}$ are also provided in Table \@ref(tab:reference-points-table). The candidate $B_0$-based LRP and USR were approximately twice as large as the candidate $B_{MSY}$-based reference points, i.e., $B_0$-based reference points were more precautionary than the $B_{MSY}$-based reference points (Table \@ref(tab:reference-points-table)).")
```
...
The base model estimates the female spawning biomass to have been on a decreasing trajectory since 2004 (Figure \@ref(fig:fig-base-sb); Table \@ref(tab:biomass-table)). The posterior median (and 95% credibility interval) female spawning biomass in `r proj_yr` is projected to be `r f(proj_bio[2] * 1000)` t (`r f(proj_bio[1] * 1000)`-`r f(proj_bio[3] * 1000)`) (Table \@ref(tab:reference-points-table)). The median projected beginning-of-year `r proj_yr` spawning biomass, which incorporates fishing mortality arising from the observed `r end_yr` catch, is considerably higher than median estimates of both the default USR of $0.8B_{MSY}$ and the default LRP of $0.4B_{MSY}$ (Figure \@ref(fig:fig-base-sb); Table \@ref(tab:reference-points-table)).

The `r proj_yr` spawning biomass was projected to be slightly below the candidate USR $0.4B_0$ and above the LRP $0.2B_0$ (Figure \@ref(fig:fig-base-sb-bo), Table \@ref(tab:reference-points-table)).
