...
Notre modélisation révèle un déclin global de la condition corporelle sur l'ensemble de la côte d'environ 2004 à 2012, une augmentation jusqu'en 2015, et un déclin ultérieur au cours des dernières années se stabilisant depuis 2019 (Fig. \@ref(fig:condition-index)). Cependant, lorsque l'on ventile les données par enquête, on constate que cette tendance générale masque les différences observées le long de la côte (Fig. \@ref(fig:condition-index-surv)). La variation dans le temps est relativement faible par rapport à la variation entre les régions (Fig. \@ref(fig:condition-index-surv)).

La condition dans le `r wcvis` est en déclin depuis 2015 environ, mais la condition dans les autres régions étudiées est restée relativement stable au cours de ces années (Fig. \@ref(fig:condition-index-surv)). En outre, la tendance à l'augmentation de l'indice sur l'ensemble de la côte entre 2012 et 2015 a été largement alimentée par le `r qcs` (Fig. \@ref(fig:condition-index-surv)). Nous pouvons également observer ces tendances en regardant les prédictions spatiales dans le temps (Fig. \@ref(fig:condition-map)).

Le lissage en profondeur indique un facteur de condition plus élevé dans les eaux plus profondes (Fig. \@ref(fig:condition-smoother)), ce qui coïncide avec un facteur de condition plus élevé dans le `r wcvis`, qui couvre des régions plus profondes que les autres enquêtes. Lorsqu'on le laisse varier dans le temps par une marche aléatoire, l'effet de la profondeur ne semble pas lié aux températures moyennes du fond enregistrées sur tous les traits à des profondeurs comprises entre 100 et 200 m pour chaque année (Fig. \@ref(fig:condition-tv)). Ceci est cohérent avec d'autres résultats selon lesquels des facteurs latents non mesurés peuvent expliquer la grande majorité de la variabilité spatio-temporelle de la condition des poissons [@lindmark2022].
...
```{r fig-condition-tv, fig.cap = ifelse(fr(), "(ref:fig-condition-tv-fr)", "(ref:fig-condition-tv-en)"), out.width = "5.0in", results = 'asis'}
