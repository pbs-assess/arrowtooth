...
**`r dcpue`**
A standard standardized commercial CPUE index was not used due to the behaviour of the fishery.
`r sp` are targeted on known grounds, and the location information is shared among fishermen, so there is a bias towards a high CPUE.
Instead, a `r dcpue` Index was suggested by stakeholders as an approach to create an index of abundance that would span every year in the assessment and be less influenced by changes in targetting behaviour than a standard commerical CPUE index. The index was constructed using catch per unit effort for a defined 'fleet' of vessels and only included tows in which 100% of `r sp` were discarded. See Appendix XX for more details.
