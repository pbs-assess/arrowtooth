...
fit <- sdmTMB(log(cond_fac) ~ 1 + s(log_depth), mesh = mesh, 
  data = ds, spatial = "off", spatiotemporal = "rw", 
...
p <- predict(fit, newdata = nd, nsim = 250)
...
out <- split(nd, nd$survey) %>% 
  purrr::map(function(.x) {
      p <- predict(fit, newdata = .x, nsim = 150)
      ind <- get_index_sims(p, 
        agg_function = function(x) mean(x), 
        area_function = function(x, area) x * area)
      ind
