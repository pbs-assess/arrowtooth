...
```{r fig-mat, fig.cap = ifelse(fr(), "(ref:fig-mat-fr)", "(ref:fig-mat-en)"), fig.asp = 1.25, out.width = "6in"}

survey_samples2 <- survey_samples 
commercial_samples2 <- commercial_samples 
# survey_samples_syn2 <- survey_samples_syn 
...
# }else{
  mat_lst <- list(
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN QCS")),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN QCS"), type = "length"),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN HS")),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN HS"), type = "length"),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN WCVI")),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN WCVI"), type = "length"),
    fit_mat_ogive(commercial_samples2),
    fit_mat_ogive(commercial_samples2, type = "length"),    
    fit_mat_ogive(survey_samples_syn2),
    fit_mat_ogive(survey_samples_syn2, type = "length"))
...
(ref:fig-mat-en2) Same as previous, but having removed those fish scored as either resting, or as the intermediate developmental stages of 'maturing' and 'developing'. The result being to include only those that are either definitively immature or capable of having produced eggs in that year.  

<!-- `r base_model$dat$start.yr` to `r base_model$dat$end.yr` for `r sp`.  -->

(ref:fig-mat-fr2) TODO

```{r fig-mat2, fig.cap = ifelse(fr(), "(ref:fig-mat-fr2)", "(ref:fig-mat-en2)"), fig.asp = 1.3, out.width = "6in"}
# which maturities to trust
# maturities_kept <- c(1,2,3,4,5,6)
maturities_kept <- c(1,4,5,6)
# remove the resting maturity code 
survey_samples_syn2 <- survey_samples_syn %>% filter(maturity_code %in% maturities_kept)
survey_samples2 <- survey_samples %>% filter(maturity_code %in% maturities_kept)
commercial_samples2 <- commercial_samples %>% filter(maturity_code %in% maturities_kept)


mat_fn2 <- file.path(data_dir, "fit_mat_ogive_output_no_resting.rds")

# if(file.exists(mat_fn2)){
#   mat_lst <- readRDS(mat_fn2)
# }else{
  mat_lst2 <- list(
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN QCS")),
    fit_mat_ogive(filter(survey_samples2, survey_abbrev %in% "SYN QCS"), type = "length"),
    ...
    fit_mat_ogive(survey_samples_syn2),
    fit_mat_ogive(survey_samples_syn2, type = "length"))
  saveRDS(mat_lst2, mat_fn2)
# }


ma_plot_lst <- list(
  plot_mat_panels(mat_lst2[[1]]) +
  coord_cartesian(xlim = c(0, 18), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[1]]),

  plot_mat_panels(mat_lst2[[2]]) +
  coord_cartesian(xlim = c(0, 82), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[2]]),

  plot_mat_panels(mat_lst2[[3]]) +
  coord_cartesian(xlim = c(0, 18), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[3]]),

  plot_mat_panels(mat_lst2[[4]]) +
  coord_cartesian(xlim = c(0, 82), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[4]]),

  plot_mat_panels(mat_lst2[[5]]) +
  coord_cartesian(xlim = c(0, 18), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[5]]),

  plot_mat_panels(mat_lst2[[6]]) +
  coord_cartesian(xlim = c(0, 82), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[6]]),

  plot_mat_panels(mat_lst2[[7]]) +
  coord_cartesian(xlim = c(0, 18), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[7]]),

  plot_mat_panels(mat_lst2[[8]]) +
  coord_cartesian(xlim = c(0, 82), ylim = c(0, 1)) +
  theme(axis.title = element_blank()) +
  ggtitle(mat_title_lst[[8]]),

  plot_mat_panels(mat_lst2[[9]]) +
  coord_cartesian(xlim = c(0, 18), ylim = c(0, 1)) +
  ggtitle(mat_title_lst[[9]]) +
  xlab(tr("Age (years)")),

  plot_mat_panels(mat_lst2[[10]]) +
  coord_cartesian(xlim = c(0, 82), ylim = c(0, 1)) +
  ggtitle(mat_title_lst[[10]]) +
  xlab(paste0(tr("Length"), " (cm)")))
