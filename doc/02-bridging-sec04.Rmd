...
```{r bridge4-phi-table-7, results = 'asis'}
param_phi_mpd_table(models$bridge_models[[4]],
                    ...
                    caption = paste0("Effective sample sizes by gear for the '",
                                     names(models$bridge_models[[4]])[7],
...
plot_selex(models$bridge_models[[4]][[7]],
           show_maturity = TRUE,
           title = names(models$bridge_models[[4]])[7]) +
  theme(plot.title = element_text(face = "bold", size = 12),
        axis.text.x = element_text(size = 6),
        strip.text.x = element_text(size = 6))
```
...
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[6]], 1)
```
...
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[6]], 2)
```
...
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[6]], 3)
```
...
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[6]], 5)
```
...
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[6]], 6)
```
