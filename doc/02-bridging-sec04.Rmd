...
```{r bridge4-phi-table-8, results = 'asis'}
param_phi_mpd_table(models$bridge_models[[4]],
                    ...
                    caption = paste0("Effective sample sizes by gear for the '",
                                     names(models$bridge_models[[4]])[8],
...
(ref:fig-bridge4-selectivities) Maximum Posterior Density (MPD) maturity ogives and selectivity estimates for all gears. The dashed blue and red lines are the male and female maturity ogives respectively, the blue and red lines with points are the yearly selectivity estimates for males and females respectively.
...
(ref:fig-bridge4-selectivity-possible-base) Closer view of the Maximum Posterior Density (MPD) maturity ogives and selectivity estimates for all gears in the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model.

```{r selectivity-possible-base, fig.cap = "(ref:fig-bridge4-selectivity-possible-base)"}
plot_selex(models$bridge_models[[4]][[8]],
           show_maturity = TRUE,
           title = names(models$bridge_models[[4]])[8]) +
  theme(plot.title = element_text(face = "bold", size = 12),
        axis.text.x = element_text(size = 6),
        strip.text.x = element_text(size = 6))
```

(ref:fig-bridge4-selectivity-possible-base-2) Closer view of the Maximum Posterior Density (MPD) maturity ogives and selectivity estimates for all gears in the 'Fix male M=0.35, female M=0.2 (two fleets, split sex)' model.
...
(ref:fig-bridge4-age-fit-1) Maximum Posterior Density (MPD) age comp fits for the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model, for the 'FT COMM' gear.

```{r bridge4-agefit-1, fig.cap = "(ref:fig-bridge4-age-fit-1)"}
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[8]], 1)
```

(ref:fig-bridge4-age-fit-2) Maximum Posterior Density (MPD) age comp fits for the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model, for the 'WB COMM' gear.

```{r bridge4-agefit-2, fig.cap = "(ref:fig-bridge4-age-fit-2)", fig.asp = 1.4}
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[8]], 2)
```

(ref:fig-bridge4-age-fit-3) Maximum Posterior Density (MPD) age comp fits for the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model, for the 'SYN QCS' gear.

```{r bridge4-agefit-3, fig.cap = "(ref:fig-bridge4-age-fit-3)"}
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[8]], 3)
```

(ref:fig-bridge4-age-fit-5) Maximum Posterior Density (MPD) age comp fits for the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model, for the 'SYN HS' gear.

```{r bridge4-agefit-5, fig.cap = "(ref:fig-bridge4-age-fit-5)"}
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[8]], 5)
```
(ref:fig-bridge4-age-fit-6) Maximum Posterior Density (MPD) age comp fits for the 'Change fishing year to Feb 21 - Feb 20 (two fleets, split sex)' model, for the 'SYN WCVI' gear.

```{r bridge4-agefit-6, fig.cap = "(ref:fig-bridge4-age-fit-6)"}
plot_agecomp_fits_splitsex(models$bridge_models[[4]][[8]], 6)
```
