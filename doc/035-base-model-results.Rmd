...
Les estimations postérieures des points de référence de la pêche du modèle de base sont fournies dans le tableau \@ref(tab:reference-points-table) et la figure \@ref(fig:fig-base-ref-points). La biomasse reproductrice non pêchée postérieure ($SB_0$) (abrégée ici en $B_0$) avait une médiane `r f(ref_pts$sbo[2] * 1000)` t et un IC à 95% allant de `r f(ref_pts$sbo[1] * 1000)` t à `r f(ref_pts$sbo[3] * 1000)` t (Tableau \@ref(tab:reference-points-table)). Les IC 95 % postérieurs pour le PRL $0{,}2B_0$ et l'RSS $0{,}4B_0$ sont également fournis dans le tableau \@ref(tab:reference-points-table).
...
L'estimation médiane a posteriori de $B_\mathrm{MSY}$ (et de l'IC à 95%), conditionnelle aux sélectivités estimées des chaluts et aux valeurs $F_\mathrm{MSY}$ qui en résultent, était `r f(ref_pts$bmsy[2] * 1000)` t (`r f(ref_pts$bmsy[1] * 1000)`--`r f(ref_pts$bmsy[3] * 1000)`) (tableau \@ref(tab:reference-points-table)). Les IC postérieurs pour le PRL $0{,}4B_\mathrm{MSY}$ et l'RSS $0{,}8B_\mathrm{MSY}$ par défaut sont également fournis dans le tableau \@ref(tab:reference-points-table). Les LRP et USR basés sur $B_0$ étaient environ quatre fois plus importants que les points de référence basés sur $B_\mathrm{MSY}$. En d'autres termes, les points de référence basés sur $B_0$ étaient plus prudents que les points de référence basés sur $B_\mathrm{MSY}$ (tableau \@ref(tab:reference-points-table)).
...
Le modèle de base estime que la biomasse reproductrice suit une trajectoire décroissante depuis 2012 (Figure \@ref(fig:fig-base-sb), Tableau \@ref(tab:biomass-table)). La biomasse de frai médiane postérieure (et l'IC à 95 %) dans `r assess_yr` devrait être `r f(proj_bio[2] * 1000)` t (`r f(proj_bio[1] * 1000)`--`r f(proj_bio[3] * 1000)`) (Tableau \@ref(tab:reference-points-table)). La biomasse reproductrice médiane projetée au début de l'année `r assess_yr`, qui incorpore la mortalité par pêche résultant de la prise observée `r end_yr`, est considérablement plus élevée que les estimations médianes à la fois de l'RSS par défaut de $0{,}8B_\mathrm{MSY}$ et du PRL par défaut de $0{,}4B_\mathrm{MSY}$ (Figure \@ref(fig:fig-base-sb), Tableau \@ref(tab:reference-points-table)). La biomasse reproductrice `r assess_yr` a été projetée légèrement en dessous de l'USR $0.4B_0$ et au-dessus du LRP $0.2B_0$ (Figure \@ref(fig:fig-base-depletion), Tableau \@ref(tab:reference-points-table)).
