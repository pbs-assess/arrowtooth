...
(ref:fig-base-f-fr) Mortalité par pêche pour le modèle de base pour les deux pêcheries au chalut pour un seul sexe. Les graphiques pour le deuxième sexe ne sont pas présentés, car ils sont identiques. La zone ombrée représente l'IC à 95 %.
...
(ref:fig-base-selex-en) Estimated and fixed selectivities by sex for the base model. The dots are median selectivity-at-age estimates; the shaded areas are the 95% CI for those estimates. Dashed lines are maturity curves, with the colours represent sex as in the legend. Single lines with no CI are fixed selectivities.

(ref:fig-base-selex-fr) Sélectivités estimées et fixées par sexe pour le modèle de base. Les points représentent les estimations médianes de la sélectivité à l'âge ; les zones ombrées représentent l'IC à 95 % de ces estimations. Les lignes en pointillé représentent les courbes de maturité, les couleurs représentant le sexe comme dans la légende. Les lignes simples sans IC sont des sélectivités fixes.
...
(ref:fig-base-ref-points-fr) Distributions postérieures pour les points de référence et autres valeurs d'intérêt pour le modèle de base. Les indices sont 1 = Chalutiers congélateurs et 2 = Chalutiers côtiers.
