...
```{r cpue-locality-bubble, fig.cap = cpue_cap_text("locality"), out.width = "6in", fig.asp = max(dfleet_sum$locality) / 70, results = 'asis'}

dfleet <- cpue_extraction()
...
```{r cpue-depth-bubble, fig.cap = cpue_cap_text("depth"), out.width = "6in", fig.asp = max(dfleet_sum$depth) / 50, results = 'asis'}
...
```{r cpue-latitude-bubble, fig.cap = cpue_cap_text("latitude"), out.width = "6in", fig.asp = max(dfleet_sum$latitude) / 60, results = 'asis'}
...
```{r cpue-vessel-bubble, fig.cap = ifelse(fr(), "(ref:fig-cpue-vessel-bubble-fr)", "(ref:fig-cpue-vessel-bubble-en)"), out.width = "6in", fig.asp = max(dfleet_sum$vessel) / 50, results = 'asis'}
...
```{r cpue-month-bubble, fig.cap = cpue_cap_text("month"), out.width = "6in", fig.asp = max(dfleet_sum$month)/30, results = 'asis'}
...
plot_fixed_effect_coefs(model[[toplot[i]]]) +
...
toplot <- which(attr(
  model, "split_labels")$formula_version == "Full standardization")
plot_random_intercepts(model[[toplot[i]]],
...
toplot <- which(attr(
  model, "split_labels")$formula_version == "Full standardization")
plot_year_locality_interactions(model[[toplot[i]]]) +
     ggtitle(params$area_name)
