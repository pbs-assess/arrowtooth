...
$\bar{R}_\mathrm{init}$ & & Average age-$\acute{a}-1$--$A$ recruitment for initialization \\
...
\begin{flalign}
  \Theta= (R_0,M_s,\overline{R},\overline{R}_\mathrm{init},\vartheta^2,\rho,\digamma_{k,t},\{\omega_t\}_{\acute{t}=1-A}^{\acute{t}=T},\{\omega_{\mathrm{init},t}\}_{t=\acute{t}-A}^{t=\acute{t}-1}) &&
  \label{eq:df16}
\end{flalign}
\begin{flalign}
...
**Recruitment model**
Recruitment for this model is a Beverton-Holt formulation:
\begin{flalign}
  R_t = \frac{s_oB_{t-k}}{1+\beta B_{t-k}}e^{\delta_{t}-0.5\tau^2}; \qquad (\mathrm{Beverton-Holt}) &&
  \label{eq:df27}
\end{flalign} which has the lognormal bias correction term
\begin{flalign}
  e^{\delta_{t}-0.5\tau^2} = n_{\omega}(\frac{1}{2}log(2\pi) + log(\sigma)) +\frac{1}{2}\frac{\sum\limits_{i=1}^{n_{\omega}} \omega[i]^2}{\sigma^2} &&
  \label{eq:df28}
\end{flalign} where $\omega$ is either $\omega_t$ or $\omega_{init,t}$
")
```
