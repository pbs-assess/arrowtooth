...
### Recrutement du stock
Cette évaluation du stock suppose un recrutement Beverton-Holt. Le recrutement annuel et la composition initiale par âge sont traités comme des variables latentes dans `r iscam`, et les résidus entre les recrutements estimés et les modèles déterministes stock-recrutement sont utilisés pour estimer la biomasse du stock reproducteur non pêché et la compensation du recrutement. Les résidus entre les recrues estimées et prédites sont donnés par:
\begin{equation}
  \delta_t = \ln(\bar{R}e^{w_t}) - \ln(R_t)
  \label{eq:df7}
\end{equation} où $R_t$ est donné par l'équation \@ref(eq:df27). Dans cette équation, $k$ est l'âge au moment du recrutement. Un terme de correction du biais pour les erreurs du processus lognormal est inclus dans l'équation \@ref(eq:df27).
...
\begin{table}[!h]
\centering
\caption{\label{tab:iscam-variables} Une liste de symboles, de constantes
et une description des variables utilisées dans `r iscam`.}
\begin{tabular}{lll}
\hline
\bf{Symbole} & \bf{Valeur} & \bf{Description} \\
\hline
\multicolumn{3}{l}{\textbf{Indices}}\\
$s$ & & Index pour le sexe\\
$a$ & & Index pour l'âge\\
$t$ & & Indice pour l'année\\
$k$ & & Index pour l'engrenage\\
$b$ & & Indice pour le bloc d'années dans la sélectivité variable dans le temps\\
\multicolumn{3}{l}{\textbf{Dimensions du modèle}}\\
$S$ & $`r base_model$dat$num.sex`$ & Nombre de sexes\\
$\acute{a}, A$ & $`r base_model$dat$start.age`$, $`r base_model$dat$end.age`$ & Classe d'âge la plus jeune et la plus âgée ($A$ est un groupe plus)\\
$\acute{t}, T$ & $`r base_model$dat$start.yr`$, $`r base_model$dat$end.yr`$ & Première et dernière année des données de captures\\
$K$ & $`r base_model$dat$num.gears`$ & Nombre d'engins, y compris les engins d'enquête\\
\multicolumn{3}{l}{\textbf{Observations (données)}}\\
$C_{k,t}$ & & captures en poids par engin $k$ en cours d'année $t$\\
$I_{k,t}$ & & indice d'abondance relative pour l'engin $k$ en cours d'année $t$\\
\multicolumn{3}{l}{\textbf{Paramètres estimés}}\\
$R_o$ & & L'âge-$\acute{a}$ recrute dans des conditions non pêchées\\
$h$ & & Rugosité de la relation stock-recrutement\\
$\bar{R}$ & & Âge moyen-$\acute{a}$ recrutement à partir de l'année $\acute{t}$ à $T$\\
$\bar{R}_\mathrm{init}$ & & Âge moyen-$\acute{a}-1$ à $A$ recrutement pour l'initialisation \\
$M_s$ & & Taux instantané de mortalité naturelle pour le sexe $s$\\
$\hat{a}_{k,s,b}, \hat{\gamma}_{k,s,b}$ & & Paramètres de sélectivité pour l'engrenage $k$, sexe $s$, bloc d'années $b$ \\
$\Gamma_{k,s,t}$ & & Logarithme de la mortalité instantanée par pêche pour l'engin $k$, le sexe $s$, l'année $t$.\\
$\omega_t$ & & L'âge-$\acute{a}$ s'écarte de $\bar{R}$ depuis des années $\acute{t}$ à $T$\\
$\omega_\mathrm{init,t}$ & & L'âge-$\acute{a}$ s'écarte de $\bar{R}_\mathrm{init}$ depuis des années $\acute{t}$\\
$q_k$ & & Paramètre de capturabilité pour l'enquête $k$\\
$\rho$ & & Fraction de la variance totale associée à l'erreur d'observation\\
$\vartheta^2$ & & Précision totale (inverse de la variance) de l'erreur totale\\
\multicolumn{3}{l}{\textbf{Écarts types}}\\
$\sigma$ & & Écart-type pour les erreurs d'observation dans l'indice d'enquête\\
$\tau$ & & Écart-type des erreurs de processus (écarts de recrutement)\\
$\sigma_C$ & & Écart-type des captures observées par engin\\
\multicolumn{3}{l}{\textbf{Résidus}}\\
$\delta_t$ & & Recrutement annuel résiduel\\
$\eta_t$ & & Erreur résiduelle dans les captures prévues\\
\multicolumn{3}{l}{\textbf{Croissance fixe \& paramètres de maturité}}\\
$l_{\infty s}$ & & Longueur asymptotique pour le sexe $s$ \\
$\acute{k}_s$ & & Coefficient de croissance de Brody pour le sexe $s$ \\
$t_{o s}$ & & Âge théorique à la longueur zéro pour le sexe $s$ \\
$\acute{a}_s$ & & Scalaire dans l'allométrie longueur-poids pour le sexe $s$\\
$\acute{b}_s$ & & Paramètre de puissance dans l'allométrie longueur-poids pour le sexe $s$\\
$\dot{a}_s$ & & Âge à 50 \% pour la maturité sexuelle $s$ \\
$\dot{\gamma}_s$ & & Écart-type à 50 \% de maturité pour le sexe $s$\\
\hline
\end{tabular}
\end{table}
\FloatBarrier
