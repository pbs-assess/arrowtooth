...
1. 100% of mortality, $Z_{t}$, occurs prior to spawning.
...
The Dirichlet Multinomial (DM) was implemented in `r iscam` for this assessment as a replacement for iterative reweighting of age data. Iterative reweighting approaches have several issues:
...
\begin{equation}
  ...
  \label{eq:dm-eff}
\end{equation} The 'saturating' parameterization of the DM was implemeted in `r iscam`. This parameterization will revert to the Multinomial distribution with sufficiently large $\beta$ (Eq. \@ref(eq:dm-eff)) i.e. $n_{eff} \simeq n$ when $\beta >> n$. It provides an upper bound on low values of $\hat{\beta}$, i.e. $n_{eff} \simeq 1 + \beta$ when $n >> \beta$.
