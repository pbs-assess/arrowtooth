...
where $N_k$ is the number of relative abundance observations for index $k$ [@walters1994]. The negative log-likelihood for relative abundance data is given by:
...
### Partial derivatives
These are the partial derivatives corresponding to components described in Section \@ref(model-staedy-state). Also, the equations used in the application of partials for $F_\mathrm{MSY}$ calculation are included in this section.
