...
Reducing the observation error by decreasing $\sigma$ from `r base_sig` to `r sens_1_2_sig` and estimating $\vartheta^2$ increased the initial value of $\vartheta^2$ from `r f(base_vartheta$init, 2)` to `r f(sens_1_2_vartheta$init, 2)` while approximately halving $\rho$ from `r f(base_rho$init, 3)` to `r f(sens_1_2_rho$init, 3)`. The median and 95% CI of the posterior for $\vartheta^2$ was `r sens_1_2_vartheta$est`. There was little effect on the absolute biomass trajectory (Figure \@ref(fig:fig-sens-variance)), but the estimate of $B_0$ was increased from `r base_sbo[1]` to `r svals[[1]][[2]]$sbo[1]` t (Figure \@ref(fig:fig-sens-variance)). The increase in the $B_0$ estimate caused a scaling downward of the relative biomass trajectory (Figure \@ref(fig:fig-sens-variance-rel)). There were no substantial changes to the index fits, age fits, or selectivities. The posterior for $h$ was estimated higher, but with a much tighter distribution tthan for the the base model (`r sens_1_2_h$est` vs. `r base_h`$est, Figure \@ref(fig:fig-sens-variance-prior-post) compared to base model, Figure \@ref(fig:fig-base-priors-posts)).

Setting the initial value for $\tau$ to `r sens_1_4_tau` and `r sens_1_3_tau`, had little effect on absolute biomass. For $\tau$ set to `r sens_1_3_tau`, the initial value of $\vartheta^2$ was `r f(sens_1_3_vartheta$init, 2)`, and the value of $\rho$ was `r f(sens_1_3_rho$init, 3)` (Appendix \@ref(app:model-desc), Eq. \@ref(eq:df17)).

For $\tau$ = `r sens_1_4_tau`, the initial values were `r f(sens_1_4_vartheta$init, 2)` and `r f(sens_1_4_rho$init, 4)` for $\vartheta^2$ and $\rho$, respectively. The estimates for $\vartheta^2$ were `r sens_1_4_vartheta$est` and `r sens_1_3_vartheta$est` for sensitivity models with $\tau$ set to `r sens_1_4_tau` and `r sens_1_3_tau` respectively. The estimates of $B_0$ were increased for both of these models when compared to the base model, which resulted in scaling down of the relative biomass trajectory with respect to the base model (Figure \@ref(fig:fig-sens-variance-rel)). The increase of $B_0$ was much greater, and had a larger CI for the $\tau$ = 1.0 model than the $\tau$ = 0.6 model (sens_1_3_sbo$est vs. sens_1_4_sbo$est).
...
The sensitivity models that estimate $M_\mathrm{male}$ with narrow and broad priors produced estimates of `r sens_2_4_m_male$est` and `r sens_2_5_m_male$est` respectively.
