...
L'ajustement de l'indice est présenté dans la figure \@ref(fig:fig-sens-geostat-index-fits). L'ajustement de l'indice basé sur la géostatistique est à peu près visuellement équivalent à l'ajustement de l'indice dans le modèle de base, mais ils ne sont pas représentés ensemble sur le même graphique parce que les indices de base sont différents.
")
```
...
Nous avons testé l'impact de la suppression de l'indice de CPUE des rejets. La trajectoire de la biomasse relative est restée similaire, avec seulement une légère augmentation de la biomasse de fin d'année (Figure \@ref(fig:fig-sens-meeting-rel)). Les ajustements des indices de l'enquête n'ont montré que des écarts mineurs par rapport aux ajustements du modèle de base (Figure \@ref(fig:fig-sens-remove-dcpue-index-fits)).
")
```
