...
cat("### Modifying prior on $M_\mathrm{Female}$ and $M_\mathrm{male}$ {#sens-m}
...
This model is clearly sensitive to the values chosen or estimated for natural mortality. The base model uses fixed values as used by several nearby juristictions [@goa2017; @goa2019; @bering2019; @goa2020; @goa2021].
")
```
...
The catchability parameters are $q_k$ where $k$ is the gear, one for each trawl fleet and survey index (`r paste(base_model$dat$gear_names, collapse = ', ')`). These parameters have a prior mean and SD. In the base model those are `r base_qk_mean` and `r base_qk_sd`, respectively.

Two sensitivity models were run to test the $q_k$ prior influence. In the first, the means for all gears were increased from `r base_qk_mean` to `r sens_qk_mean`, and the SD remained at `r sens_qk_sd`. In the second, the prior was broadened by setting the SD for all the gears to `r sens_qkp_sd`. The means for that model remained at `r sens_qkp_mean`.

The catchability estimates for each gear did not vary substantially between models (Figure \@ref(fig:fig-sens-q-q)).
