...
It appears from the index fit to the discard CPUE index in the base model that the model is heavily influenced by it (Figure \@ref(fig:fig-base-index-fits)). To test this, the discard CPUE index was completely removed from the model.

The relative biomass trajectory was not affected much at all, wih only a very slight increase in end year biomass (Figure \@ref(fig:fig-sens-meeting-rel)). The fits for the survey indices showed only minute departures from the base model fits (Figure \@ref(fig:fig-sens-remove-dcpue-index-fits)).
")
```
...
Removing 'developing' and restin' from the maturity ogive produced a better-fitting ogive (Figure \@ref(fig:fig-mat)) and resulted in a slightly lower final year absolute biomass compared to the base model, but the same relative biomass (Figures \@ref(fig:fig-sens-meeting) and \@ref(fig:fig-sens-meeting-rel)). This maturity should be considered for the base model in the next assessment. Since it made no difference on the results, and because it was worked up following the review meeting, we did not attempt to include it in the base model for this assessment .
")
```
