...
This sensitivity came about in the `r la` cycle, where it was found that the estimated selectivity curves were all to the right of the maturiy ogive (Figure 17, @grandin2017). This caused the value of $F_\mathrm{MSY}$ to be very large and essentially give the advice that an unlimited amount of catch could be taken without affecting the stock. We repeat it here, as the same situation has arisen and to compare this model with the single sex one from the `r la`.

TODO: complete this once model is done Sept 23 4PM

For this model structure, there is virtually no difference in the absolute biomass and $B_0$ estimates between the base model and the model where selectivities equal maturity (Figures \@ref(fig:fig-sens-selex) and \@ref(fig:fig-sens-selex-rel)).
...
In an attempt to improve the fit of the `r qcs` index (Figure @\ref(fig:fig-base-index-fits)), we implemented time-varying selectivity in `r iscam` and ran the model with the `r qcs` having three blocks of selectivity, 2003--2010, 2011--2016, and 2017--2021. This did improve the latter part of the index fit (Figure \@ref(fig:fig-sens-selex-qcs)). This came at the expense of a very poor estimate of selectivity. The selectivity estimates for ...
