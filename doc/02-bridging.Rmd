...
The absolute and relative biomass plots (Figure \@ref(fig:bridge-biomass)) show that the data that were available in 2014 when the 2015 stock assessment was being prepared produced a model with increasing biomass, partly due to the indices being at high levels and partly because the recruitment (Figure \@ref(fig:bridge-recruitment)) was being predicted as quite large for 2012. The large 2012 recruitment estimate was due to the high uncertainty from lack of data in that model. This can be seen in Figure 21 of the 2015 assessment (@grandin2017) which shows the large uncertainty around the 2012 recruitment for the MCMC estimates of recruitment. iSCAM will add biomass through an increased recruitment event when data are limited and biomass has to be accounted for to fit the indices. There were 18 years of catch data and only a few index points available at the time the 2015 model was run.
...
All the bridging models were able to estimate all selectivities for gears which had age compositions. The Hecate Strait Multispecies Assemblage survey (OTHER HSMSA) and Discard CPUE (DCPUE) indices do not have age compositions and have fixed selectivity values. The DCPUE index is a time series of the yearly sum of discarded fish on non-targeted tows. None of those discarded fish were sampled for age. They were sampled for sex prior to being discarded and those data were used in the proportion female analysis to determine proportion of catch for the single sex models.

The WCHG Synoptic survey had estimates of selectivity which are quite a bit different than the other indices. The age-at-50% ranged from 25.73 for the '`r names(models$bridge_models[[mdl_grp]])[4]`' model to 3.25 and 2.13 for the '`r names(models$bridge_models[[mdl_grp]])[9]`' and '`r names(models$bridge_models[[mdl_grp]])[10]`' models.

In the previous iteration of this bridging, the Dirichlet Multinomial likelihood was not implemented yet and all models were using the Multivariate Logistic likelihood, which did not allow for estimation of all selectivities simultaneously for models with two fleets and split sex.
...
The age composition likelihood for the 2015 model was a Multivariate Logistic Normal (MLN) which applies 'self-weighting' to the age comps based on the conditional maximum likelihood variances in the age proportions. The MVL did not perform well in the models with two fleets and two sexes. Selectivity on several of the Synoptic surveys had to be fixed in order to get reasonable parameter estimates and/or model convergence. Because of this we decided to implement the Dirichlet Multinomial likelihood into iSCAM.

The Dirichlet-Multinomial ensures that the uncertainty associated with the composition weighting is included in any measures of stock assessment uncertainty, and that manual weighting and iterative reweighting methods are avoided. It also means that sensitivity and retrospective models will have the correct weighting automatically, whereas in the past the base model weightings were used for sensitivities and retrospectives. The 'saturating' DM method (@thorson2017) adds one new estimated parameter for each gear/year combination with age composition data to the model.

Tables \@ref(tab:bridge-phi-table-9) and \@ref(tab:bridge-phi-table-10) show the effective sample sizes for the models '`r names(models$bridge_models[[mdl_grp]])[mdl_ind]`' and '`r names(models$bridge_models[[mdl_grp]])[mdl_ind + 1]`' respectively. The effective samples size in the table is calculated as $(N_{eff} + N_{eff} * e^{log\_phi}) / (N_{eff} + e^{log\_phi})$, where $log\_phi$ is an estimated parameter of the DM likelihood and $N_{eff}$ is set before running the model. A value of $N_{eff} = 130$ was arrived upon by the following iterative method.

Start with $N_{eff} = 200$.

1. Run the MPD model and check the estimated values of $log\_phi$, if none have hit the upper bound, set $N_{eff}$ to a smaller value, 50 less than before and repeat until upper bounds are hit.

1. If upper bounds were hit, increase the $N_{eff}$ to a higher value, 10 more than before and repeat until upper bounds are not hit.

1. Once upper bounds are no longer hit, that is the value to use as an initial $N_{eff}$.

The lower the initial $N_{eff}$, the smaller the likelihood components for age comps in Table \@ref(tab:bridge-likelihoods).

The effective sample sizes for the first model, where M is estimated are slightly higher overall than for the second, where M was fixed. The likelihoods for the age comps for the estimated M model are also a little less that for the fixed M model (Table \@ref(tab:bridge-likelihoods)).
...
The WCHG survey index had several (related) issues, including lack of a decent age fit and inability for the model to estimate reasonable selectivity. We recommend leaving it out of this model. Removing it has virtually no affect on the outputs of the model.
