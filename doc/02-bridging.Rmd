...
The WCHG Synoptic survey had estimates of selectivity which are quite a bit different than the other indices. The age-at-50% ranged from 25.73 for the '`r names(models$bridge_models[[mdl_grp]])[4]`' model to 3.25 for the '`r names(models$bridge_models[[mdl_grp]])[9]`' model (Table \@ref(tab:bridge-sel-param-table) and Figures \@ref(fig:bridge-selectivities) - \@ref(fig:selectivity-possible-base-2)).

In the previous iteration of this bridging analysis, the Dirichlet Multinomial likelihood for age composition weighting was not implemented yet and all models were using the Multivariate Logistic likelihood, which did not allow for estimation of all selectivities simultaneously for models with two fleets and split sex.
...
The age composition likelihood for the 2015 model was a Multivariate Logistic Normal (MLN) which applies 'self-weighting' to the age comps based on the conditional maximum likelihood variances in the age proportions. The MLN did not perform well in the models with two fleets and two sexes. Selectivity on several of the Synoptic surveys had to be fixed in order to get reasonable parameter estimates and/or model convergence. Due to these issues, we decided to implement the Dirichlet Multinomial likelihood in iSCAM.

The Dirichlet-Multinomial ensures that the uncertainty associated with the composition weighting is included in any measures of stock assessment uncertainty, and that manual weighting and iterative reweighting methods are avoided. It also means that sensitivity and retrospective models will have the correct weighting automatically, whereas in the past the base model weightings were used for sensitivities and retrospectives because manually reweighting them all took too much time. We implemented the 'saturating' DM method (@thorson2017) which adds one new estimated parameter for each gear/year combination with age composition data to the model.
...
gear <- 1
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]],
                             gear = gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", FT COMM"),
                             type = "pearson",
                             pearson_type = "age") +
...
gear <- 2
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]], gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", WB COMM"),
                             type = "pearson",
                             pearson_type = "age") +
...
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]], gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", SYN QCS"),
                             type = "pearson",
                             pearson_type = "age") +
    coord_cartesian(ylim = c(-1, 1)),
...
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]], gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", SYN HS"),
                             type = "pearson",
                             pearson_type = "age") +
    coord_cartesian(ylim = c(-1, 1)),
...
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]], gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", SYN WCVI"),
                             type = "pearson",
                             pearson_type = "age") +
    coord_cartesian(ylim = c(-1, 1)),
...
cowplot::plot_grid(
  plot_agecomp_fits_splitsex(models$bridge_models[[mdl_grp]][[mdl_ind]], gear,
                             model_name = paste(names(models$bridge_models[[mdl_grp]])[mdl_ind], ", SYN WCHG"),
                             type = "pearson",
                             pearson_type = "age") +
    coord_cartesian(ylim = c(-1, 1)),
