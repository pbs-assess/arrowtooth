...
Each model in this section is based on the previous one with only one change made so incremental changes can be tracked. The starting model here (*Update 2019 data*) is repeated from Section \@ref(single-fleet-female-only-models) so that relative differences in trends can be seen.
