...
Negative log-likelihood (NLL) components of the objective function are listed in Table \@ref(tab:bridge-likelihoods). There is a clear change in the NLL values for the survey indices when the age composition likelihood is changed to Dirichlet-Multinomial, with large positives instead of smaller negative values. These larger positives mean that the age compositions are contributing less to the likelihood than the Multivariate Logistic likelihood method. This is typical of the DM method, for example in the Pacific hake model in 2018 the NLL was approximately 555. The other major difference in these likelihoods occurs when the model is changed to a split-sex configuration and they double. That change is due to there being age compositions for both sexes.
...
The age composition likelihood for the 2015 model was a Multivariate Logistic Normal (MLN) which applies 'self-weighting' to the age comps based on the conditional maximum likelihood variances in the age proportions. The MVL did not perform well with the addition of two fleets and two sexes. Selectivity on several of the Synoptic surveys had to be fixed in order to get reasonable parameter estimates and/or model convergence. Because of this we decided to implement the Dirichlet Multinomial likelihood into iSCAM.

The Dirichlet-Multinomial ensures that the uncertainty associated with the composition weighting is included in any measures of stock assessment uncertainty, and that manual weighting and iterative reweighting methods are avoided. It also means that sensitivity models will have the correct weighting automatically, whereas in the past the base model weighting was used. The DM method adds some new parameters to the model which are estimated.

Tables \@ref(tab:bridge-phi-table-9) and \@ref(tab:bridge-phi-table-10) show the effective sample sizes for the models '`r names(models$bridge_models[[mdl_grp]])[mdl_ind]`' and '`r names(models$bridge_models[[mdl_grp]])[mdl_ind + 1]`' respectively. The effective samples size is calculated as $(200 + 200 * e^{log\_phi}) / (200 + e^{log\_phi})$. The value 200 is a constant that was set as the initial guess for $phi$ in the DM likelihoods. The effective sample sizes for the first model, where M is estimated are slightly higher overall than for the second, where M was fixed.
...
```{r bridge-selectivities, fig.cap = "(ref:fig-bridge-selectivities)", out.height = "7in"}
