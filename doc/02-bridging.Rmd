...
b("Each model in this list is based on the previous one with only one change made so incremental changes can be tracked.
...
b("Note that the extra large recruitment for 2014 which is not fully exposed in the figure (~3.5 million) was never included in the model output advice in 2015 because iSCAM overwrites the final year recruitment with an average recruitment in the projection model for the final year.")
```
...
```{r bridging-para-07-en, eval = !fr(), results = 'asis'}
b("# Index fits
...
```
```{r bridging-para-07-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-07-en>>
```

```{r bridging-para-08-en, eval = !fr(), results = 'asis'}
b("The West Coast Vancouver Island Synoptic survey (SYN WCVI) has some large swings in data from low biomass to high biomass in subsequent survey years. This is extremely hard for a model to fit and this model splits the difference with its fit, as it should, and also follows the recent downward trend observed by the surveys.")
```
```{r bridging-para-08-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-08-en>>
```

```{r bridging-para-09-en, eval = !fr(), results = 'asis'}
b("The Discard CPUE (DCPUE) has the most data points of any index of abundance and gives the clearest view of the biomass trend. It is the best-fitting index in this model. This index is clearly driving the model fits as the same pattern can be seen in fits to other survey indices.")
```
```{r bridging-para-09-fr, eval = !fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-09-en>>
```

```{r bridging-para-10-en, eval = !fr(), results = 'asis'}
...
```
```{r bridging-para-10-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-10-en>>
```
...
```{r bridging-para-12-en, eval = !fr(), results = 'asis'}
...
```
```{r bridging-para-12-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-12-en>>
```

```{r bridging-para-13-en, eval = !fr(), results = 'asis'}
b("The other notable impact model choice has on parameter estimates is fixing the natural mortality parameters to the same values used in the Gulf of Alaska assessment (Male = 0.2, Female = 0.35). This scales all recruitment parameters up. These increases are compensation by the model to account for the increased natural mortality placed on the stock.")
```
```{r bridging-para-13-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-13-en>>
```

```{r bridging-para-14-en, eval = !fr(), results = 'asis'}
b("Negative log-likelihood (NLL) components of the objective function are listed in Table \\@ref(tab:bridge-likelihoods). There is a clear change in the NLL values for the age compositions when the age composition likelihood is changed to the Dirichlet-Multinomial (row 5), with large positives instead of negative values. These larger positives are an artifact of the way in which the DM likelihoods are calculated and should not be compared with the smaller NLLs for survey indices. The NLLs should be compared between models for the same likelihood components. This is typical of the DM method, for example in the Pacific hake model in 2021 the NLL for the survey age compositions is 555.99 for the base model (Table 31, @hake2021) when other NLLs were one or more orders of magnitude smaller.")
```
```{r bridging-para-14-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-14-en>>
```

```{r bridging-para-15-en, eval = !fr(), results = 'asis'}
b("The other major difference in the likelihoods occurs when the model is changed to a split-sex configuration (row 8). That change is simply due to there being age compositions for both sexes, so additional likelihoods are calculated for each year and added to the total. The change amounts to roughly double.")
```
```{r bridging-para-15-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-15-en>>
```
...
```{r bridging-para-16-en, eval = !fr(), results = 'asis'}
b("# Selectivities
...
```
```{r bridging-para-16-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-16-en>>
```

```{r bridging-para-17-en, eval = !fr(), results = 'asis'}
b("For the split-sex models, the WCHG Synoptic survey had estimates of selectivity which are quite a bit different than the other indices. The age-at-50% ranged dropped from approximately 7 to 3 for the models which include split-sex (Table \\@ref(tab:bridge-sel-param-table) and Figures \\@ref(fig:bridge-selectivities) -- \\@ref(fig:selectivity-possible-base-2)).")
```
```{r bridging-para-17-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-17-en>>
```

```{r bridging-para-18-en, eval = !fr(), results = 'asis'}
b("In previous iterations of this bridging analysis, the Dirichlet Multinomial likelihood for age composition weighting was not implemented yet and all models were using the Multivariate Logistic likelihood, which did not allow for estimation of all selectivities simultaneously for models with two fleets and split sex.")
```
```{r bridging-para-18-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-18-en>>
```

```{r bridging-para-19-en, eval = !fr(), results = 'asis'}
b("## Selectivity figures for all models")
```
```{r bridging-para-19-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-19-en>>
```
...
```{r bridging-para-20-en, eval = !fr(), results = 'asis'}
b("## Selectivity fits for select models")
```
```{r bridging-para-20-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-20-en>>
```
...
```{r bridging-para-21-en, eval = !fr(), results = 'asis'}
b("# Age compositions and effective sample size
...
```
```{r bridging-para-21-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-21-en>>
```

```{r bridging-para-22-en, eval = !fr(), results = 'asis'}
b("Incorporating the DM likelihood ensures that the uncertainty associated with the composition weighting is included in any measures of stock assessment uncertainty, and that manual weighting and iterative reweighting methods are avoided. It also means that sensitivity and retrospective models will have the correct weighting automatically, whereas with other methods the base model weightings were typically used for sensitivities and retrospectives because manually running the reweighting methods outside the model took too much time. We implemented the 'saturating' DM method (@thorson2017) which adds one new estimated parameter for each gear/year combination with age composition data to the model.")
```
```{r bridging-para-22-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-22-en>>
```

```{r bridging-para-23-en, eval = !fr(), results = 'asis'}
b("Tables \\@ref(tab:bridge-phi-table-9) and \\@ref(tab:bridge-phi-table-10) show the actual data-based sample sizes, the estimates of the DM parameter $log(\\phi)$, and the effective sample sizes calculated using $log(\\phi)$ for the models '", names(models$bridge_models[[mdl_grp]])[9], "' and '", names(models$bridge_models[[mdl_grp]])[10], "' respectively. The effective sample size is calculated as $N_{eff} = (N + N * e^{log(\\phi)}) / (N + e^{log(\\phi)})$, where $log(\\phi)$ is an estimated parameter of the DM likelihood and $N$ is the actual sample size in the data (Eq. 9 in @thorson2017). The sample sizes used are actually specimen counts which is permitted by the ().")
```
```{r bridging-para-23-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-23-en>>
```

```{r bridging-para-24-en, eval = !fr(), results = 'asis'}
b("The effective sample sizes for the first model, where $M$ is estimated are slightly higher overall than for the second, where $M$ was fixed. The likelihoods for the age comps for the estimated $M$ model are less than for the fixed $M$ model (Table \\@ref(tab:bridge-likelihoods)).")
```
```{r bridging-para-24-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-24-en>>
```
...
```{r bridging-para-25-en, eval = !fr(), results = 'asis'}
b("## Age comp fits, model ", names(models$bridge_models[[mdl_grp]])[mdl_ind], 
...
```
```{r bridging-para-25-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-25-en>>
```
...
```{r bridging-para-26-en, eval = !fr(), results = 'asis'}
b("## Age comp fits, model ", names(models$bridge_models[[mdl_grp]])[mdl_ind], 
...
```
```{r bridging-para-26-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-26-en>>
```
...
```{r bridging-para-27-en, eval = !fr(), results = 'asis'}
b("## Age composition residuals
  
### Age composition residuals for  '", names(models$bridge_models[[mdl_grp]])[mdl_ind], "' model")
```
```{r bridging-para-27-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-27-en>>
```
...
```{r bridging-para-28-en, eval = !fr(), results = 'asis'}
b("### Age composition residuals for  '", names(models$bridge_models[[mdl_grp]])[mdl_ind], "' model")
```
```{r bridging-para-28-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-28-en>>
```
...
```{r bridging-para-29-en, eval = !fr(), results = 'asis'}
b("## Age composition data for all models

These are the age composition data included in all models.")
```
```{r bridging-para-29-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-29-en>>
```
...
```{r bridging-para-30-en, eval = !fr(), results = 'asis'}
b("# Summary
  
Considering all the outputs from the MPD models, and that this species is sexually dimorphic with two different types of vessels targeting it, we feel that we should use the '", names(models$bridge_models[[mdl_grp]])[9], "' model as the base (or reference) model for the 2021 stock assessment.")
```
```{r bridging-para-30-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-30-en>>
```

```{r bridging-para-31-en, eval = !fr(), results = 'asis'}
b("Using that model as a base, we fixed the natural mortality to 0.2 for males and 0.35 for females in the model '", names(models$bridge_models[[mdl_grp]])[10], "' just as they do in the Gulf of Alaska assessment. When compared side-by-side with the model that estimates M, we can see several reasons to choose the model that estimates M. The Dirichlet Multinomial effective sample sizes are larger for the estimated M model, and the age composition likelihoods are a little lower. Also, the estimated standard deviation at 50% for the WCHG selectivity is better for the estimated M model. Also, it is good practice to allow M to be estimated by the model when possible.")
```
```{r bridging-para-31-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-31-en>>
```

```{r bridging-para-32-en, eval = !fr(), results = 'asis'}
b("The QCS survey index is not fit well at all in the recent part of the time series. It can either be left in or removed, it doesn't make much difference in the model outputs, but there are approximately 640 ages contributing to the age compositions for that index.")
```
```{r bridging-para-32-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-32-en>>
```

```{r bridging-para-33-en, eval = !fr(), results = 'asis'}
b("The WCHG survey index had several (related) issues, including lack of a decent age fit and inability for the model to estimate reasonable selectivity and catchability. We recommend leaving it out of this model. Removing it has virtually no affect on the outputs of the model.

\\
\\
**We would like to proceed with the '", names(models$bridge_models[[mdl_grp]])[9], "' model with the WCHG survey index and age comps removed as our base model for this stock assessment.**")
```
```{r bridging-para-33-fr, eval = fr(), results = 'asis', needs_trans = TRUE}
<<bridging-para-33-en>>
```
