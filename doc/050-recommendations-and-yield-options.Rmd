...
Performance measureswere calculated overa sequence of alternative `r end_yr` projected catch levels and are based on one-year projections to `r assess_yr`. Projected, bias-corrected log recruitment anomalies in `r end_yr` and `r assess_yr` were drawn randomly from a normal distribution, $~N(0,\tau)$.

Median (and 95% credibility interval) posterior estimates of reference points (MSY and $B_0$-based reference points) and benchmarks were provided in Table \@ref(tab:reference-points-table). Decision tables are presented showing predicted probabilities of undesirable states under alternative `r assess_yr` projected catch levels. An undesirable biomass-based performance measure is defined to occur when the `r proj_yr` projected female spawning biomass is below the candidate reference point or benchmark, i.e., the ratio `r paste0('$B_{', proj_yr, '}/B_{ReferencePoint} < 1$')`. An undesirable fishing mortality-based performance measure is defined to occur when projected `r assess_yr` fishing mortality is above the reference point, i.e.,
`r paste0('$F_{', assess_yr, '}/F_{ReferencePoint} > 1$')`.
...
The model-predicted probability of the `r proj_yr` female spawning biomass being below the `r assess_yr` female spawning biomass ranged from `r prob_proj_less_assess_0` under zero `r assess_yr` catch to `r prob_proj_less_assess_10` under 10,000 t of catch, which is double the current total TAC. At 50,000 t, the probability is `r prob_proj_less_assess_50`. The TAC which is closest to 0.5 probability of the biomass declining from `r assess_yr` to `r proj_yr` while still being below it is `r prob_less_50_50$tac` t, at a probability of `r val_less_50_50`.
