...
```{r proportion-female}
# Commercial fishery
ac_coast <- tidy_ages_weighted(dat$commercial_samples,
                               sample_type = "commercial",
                               dat_catch = dat$catch)
lc_coast <- tidy_lengths_weighted(dat$commercial_samples,
                                  sample_type = "commercial",
                                  dat_catch = dat$catch)
aprop_coast <- calc_female_props(ac_coast, type = "age")
lprop_coast <- calc_female_props(lc_coast, type = "length")
comm_props <- full_join(aprop_coast, lprop_coast, by = "year") |> 
  select(-c(type.x, type.y)) |> 
  rename(`Proportion female by age` = female_prop.x,
         `Proportion female by length` = female_prop.y)
comm_prop_fem_by_age <- mean(pull(comm_props[, 2]), na.rm = TRUE)
comm_prop_fem_by_length <- mean(pull(comm_props[, 3]), na.rm = TRUE)

# Synoptic surveys
as_coast <- survey_samples_syn %>%
  tidy_ages_weighted(sample_type = "survey",
                     dat_survey_sets = dat$survey_sets) %>% 
  mutate(survey_abbrev = as.character(survey_abbrev))
ls_coast <- survey_samples_syn |> 
  tidy_lengths_weighted(sample_type = "survey",
                        dat_survey_sets = dat$survey_sets)
aprop_coast <- calc_female_props(as_coast, type = "age")
lprop_coast <- calc_female_props(ls_coast, type = "length")
surv_props <- full_join(aprop_coast, lprop_coast, by = "year") |> 
  select(-c(type.x, type.y)) |> 
  rename(`Proportion female by age` = female_prop.x,
         `Proportion female by length` = female_prop.y)
surv_prop_fem_by_age <- mean(pull(surv_props[, 2]), na.rm = TRUE)
surv_prop_fem_by_length <- mean(pull(surv_props[, 3]), na.rm = TRUE)

prop_female <- mean(comm_prop_fem_by_age,
                    comm_prop_fem_by_length,
                    surv_prop_fem_by_age,
                    surv_prop_fem_by_length)
