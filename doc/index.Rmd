...
base_inp_params <- as_tibble(rownames_to_column(as.data.frame(base_model$ctl$params), var = "param"))
base_vartheta_row <- base_inp_params |> filter(param == "vartheta")
base_vartheta <- f(base_vartheta_row |> pull(ival), 2)
base_rho_row <- base_inp_params |> filter(param == "rho")
base_rho <- f(base_rho_row |> pull(ival), 3)
base_vartheta_est_raw <- f(as_tibble(base_model$mcmccalcs$params_quants)$vartheta, 2)
base_vartheta_est <- paste0(base_vartheta_est_raw[2],
                            " (", base_vartheta_est_raw[1], "--", base_vartheta_est_raw[3], ")")

sens01_inp_params <- as_tibble(rownames_to_column(as.data.frame(models$sens_grps[[1]][[2]]$ctl$params), var = "param"))
sens01_vartheta_row <- sens01_inp_params |> filter(param == "vartheta")
sens01_vartheta <- f(sens01_vartheta_row |> pull(ival), 2)
sens01_rho_row <- sens01_inp_params |> filter(param == "rho")
sens01_rho <- f(sens01_rho_row |> pull(ival), 3)
sens01_vartheta_est_raw <- f(as_tibble(models$sens_grps[[1]][[2]]$mcmccalcs$params_quants)$vartheta, 2)
sens01_vartheta_est <- paste0(sens01_vartheta_est_raw[2],
                            " (", sens01_vartheta_est_raw[1], "--", sens01_vartheta_est_raw[3], ")")

sens02_inp_params <- as_tibble(rownames_to_column(as.data.frame(models$sens_grps[[1]][[3]]$ctl$params), var = "param"))
sens02_vartheta_row <- sens02_inp_params |> filter(param == "vartheta")
sens02_vartheta <- f(sens02_vartheta_row |> pull(ival), 2)
sens02_rho_row <- sens02_inp_params |> filter(param == "rho")
sens02_rho <- f(sens02_rho_row |> pull(ival), 3)
sens02_vartheta_est_raw <- f(as_tibble(models$sens_grps[[1]][[3]]$mcmccalcs$params_quants)$vartheta, 2)
sens02_vartheta_est <- paste0(sens02_vartheta_est_raw[2],
                            " (", sens02_vartheta_est_raw[1], "--", sens02_vartheta_est_raw[3], ")")

sens03_inp_params <- as_tibble(rownames_to_column(as.data.frame(models$sens_grps[[1]][[4]]$ctl$params), var = "param"))
sens03_vartheta_row <- sens03_inp_params |> filter(param == "vartheta")
sens03_vartheta <- f(sens03_vartheta_row |> pull(ival), 2)
sens03_rho_row <- sens03_inp_params |> filter(param == "rho")
sens03_rho <- f(sens03_rho_row |> pull(ival), 3)
sens03_vartheta_est_raw <- f(as_tibble(models$sens_grps[[1]][[4]]$mcmccalcs$params_quants)$vartheta, 2)
sens03_vartheta_est <- paste0(sens03_vartheta_est_raw[2],
                            " (", sens03_vartheta_est_raw[1], "--", sens03_vartheta_est_raw[3], ")")

vartheta_sens_sigma <- f(models$sens_grps[[1]][[2]]$ctl$params[rownames(base_model$ctl$params) == "vartheta", ][1], 2)
rho_sens_sigma <- f(models$sens_grps[[1]][[2]]$ctl$params[rownames(base_model$ctl$params) == "rho", ][1], 3)
vartheta_est_sens_sigma <- f(as_tibble(models$sens_grps[[1]][[2]]$mcmccalcs$params_quants)$vartheta, 2)

# Steepness prior values
p1_base <- base_model$ctl$params[rownames(base_model$ctl$params) == "h", ]["p1"]
p2_base <- base_model$ctl$params[rownames(base_model$ctl$params) == "h", ]["p2"]
h_prior_base <- f(calc_beta_mean_cv(p1_base, p2_base), 2)
p1_sens01 <- models$sens_grps[[1]][[5]]$ctl$params[rownames(models$sens_grps[[1]][[5]]$ctl$params) == "h", ]["p1"]
p2_sens01 <- models$sens_grps[[1]][[5]]$ctl$params[rownames(models$sens_grps[[1]][[5]]$ctl$params) == "h", ]["p2"]
h_prior_sens01 <- f(calc_beta_mean_cv(setNames(p1_sens01, NULL), setNames(p2_sens01, NULL)), 2)

# M_male priors and estimates
sens_male_inp_params <- as_tibble(rownames_to_column(as.data.frame(models$sens_grps[[2]][[3]]$ctl$params), var = "param"))
sens_male_m_row <- sens_male_inp_params |> filter(param == "log_m_male")
sens_male_m_ival <- f(sens_male_m_row |> pull(ival) |> exp(), 2)
sens_male_m_tight_est_raw <- f(as_tibble(models$sens_grps[[2]][[4]]$mcmccalcs$params_quants)$m_sex1, 2)
sens_male_m_tight_est <- paste0(sens_male_m_tight_est_raw[2],
                            " (", sens_male_m_tight_est_raw[1], "--", sens_male_m_tight_est_raw[3], ")")
sens_male_m_loose_est_raw <- f(as_tibble(models$sens_grps[[2]][[5]]$mcmccalcs$params_quants)$m_sex2, 2)
sens_male_m_loose_est <- paste0(sens_male_m_loose_est_raw[2],
                            " (", sens_male_m_loose_est_raw[1], "--", sens_male_m_loose_est_raw[3], ")")
