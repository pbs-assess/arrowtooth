...
french_abstract: |
    ...
    Le modèle de base présenté dans cette évaluation estime la biomasse féconde médiane de 2022 à 59 493 tonnes. Des analyses de sensibilité ont été effectuées pour tester les effets des paramètres fixes et des distributions de probabilité antérieures supposées sur les résultats du modèle. Dans tous les scénarios, il y avait une forte confusion entre les paramètres représentant la productivité et l'échelle de la population, ce qui indique qu'il y avait peu d'informations dans les courtes séries temporelles de données disponibles pour résoudre l'échelle de la population.

    Le modèle de base estime que la biomasse reproductrice suit une trajectoire décroissante depuis environ 2012. Les points de référence basés sur le rendement maximal durable RMD ont été fortement influencés par les estimations de la sélectivité dans les pêcheries au chalut. Des points de référence basés sur des fractions de $B_0$ (biomasse féconde non pêchée) ont été choisis à la place, comme cela a été fait dans la dernière évaluation. En 2022, on prévoyait que la biomasse reproductrice serait inférieure au USR de $0,4B_0$ et supérieure au LRP de $0,2B_0$. Il y avait une probabilité de 15,4 % que la biomasse féconde soit inférieure du PLR de $0,2B_0$ en 2022. Des analyses de sensibilité ont été effectuées pour tester les effets des paramètres fixes, des distributions de probabilité antérieures et du traitement des données d'entrée sur les résultats du modèle. Dans la plupart des modèles de sensibilité, les diagnostics MCMC étaient médiocres ou les estimations de la sélectivité et/ou de la capturabilité n'étaient pas fiables.

    Une série d'exécutions rétrospectives du modèle remontant à huit ans a indiqué un point de rupture distinct lorsque les données à partir de 2019 ont été ajoutées. À ce stade, les baisses de biomasse reproductrice estimée dues aux données deviennent évidentes à partir de 2012.

    Les conseils de gestion sont fournis sous la forme de tableaux de décision qui prévoient les impacts d'une gamme de niveaux de capture pour 2022 sur l'état du stock de limande à dents de flèche par rapport à ces points de référence. Le tableau de décision du modèle de base suggère qu'une prise de 4 000 t en 2022 (1 000 t de moins que le TAC de 2022) ferait en sorte que la biomasse de 2023 soit inférieure au RSU de $0,4 B_0$ avec une probabilité de 62 %. La même capture ferait en sorte que la biomasse de 2023 soit inférieure au PRL de $0,2B_0$ avec une probabilité de moins de 1 %.
  
    L'ampleur des prises et des rejets avant 1996 est une source importante d'incertitude dans cette évaluation, alors qu'elle pourrait fournir des renseignements essentiels sur l'ampleur et la productivité de ce stock. L'utilisation d'une enquête cousue pour remplacer les indices distincts de l'enquête synoptique pourrait aider à résoudre certains problèmes d'ajustement de l'indice de l'enquête synoptique du détroit de la Reine-Charlotte, qui a un taux de déclin plus faible que les autres indices de l'enquête, ce qui rend difficile son ajustement.
