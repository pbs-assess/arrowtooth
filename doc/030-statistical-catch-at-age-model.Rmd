...
The $F_\mathrm{MSY}$ (and annual harvest rate $U_\mathrm{MSY}$) are estimated to be very large in this model due to selectivity being greater than maturity, as described in Section \textbf{TODO}. We therefore present $B_0$-based reference points for `r sp` that are less reliant on estimated selectivity. We suggest an USR = $0.4B_0$ and a LRP = $0.2B_0$. These thresholds are consistent with biomass targets and limits in place in other jusrisdictions including Australia [@smith2007] and the U.S.A. [@restropo1998]. They were also used in the last assessment for `r sp` in `r bc` [@grandin2017].
")
```
