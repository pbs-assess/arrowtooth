...
#' The 'Discard CPUE' index must be multiplied by the proportions female
#' from the commercial fishery so `comm_samples` is required
#'
#' @details The CV can be calculated as CV = SD / MEAN or from the Standard
#' error: CV = sqrt(exp(SE^2) - 1)
#'
#' @rdname props_surv
#' @param survey_index Survey index data frame as output by
#' [gfdata::get_survey_index()]
#' @param iphc The IPHC index as read in from iphc-survey-index.rds
#' @param discard_cpue The discard CPUE index as read from the
#' 'cpue-predictions*' file
#' @param stitched_syn The stitched synoptic index as read in from
#' 'stitched-syn-index.rds'
...
#' export_survey_indices(si, start_year = 1996, end_year = 2023, species_category = 1)
#' }
