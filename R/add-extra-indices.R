...
#'
#' @param surv_index A survey index object as returned by
#' [gfdata::get_survey_index()]
#' @param iphc The IPHC index data
#' @param discard_cpue The Discard CPUE index data
#' @param stitched_syn The Stitched Synoptic index data
#'
#' @return A survey index object as returned by
#' [gfdata::get_survey_index()], with the additional indices added
#' @export
  ...
  if(!is.null(iphc)){
    iphc <- iphc %>%
      transmute(year,
                ...
                survey_series_desc = paste0("International Pacific Halibut ",
                                            "Commission Fishery-Independent ",
                                            "Setline Survey"))
    if(ncol(surv_index) != ncol(iphc)){
      stop("Check the number of columns in surv_index, it does not match ",
           "the iphc extraction")
    }
